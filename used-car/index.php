  ...
  <div class="mx-auto max-w-3xl px-6">
    ...
    <div class="mt-4 flex gap-2">
      <?php foreach ($filters as $f): $on = ($f === $active_filter); ?>
        <a href="?<?= http_build_query(['f' => $f, 'q' => $query]) ?>" class="px-5 py-2 text-xs font-black border transition <?= $on ? 'bg-neutral-900 text-white border-neutral-900' : 'border-neutral-200 text-neutral-500 hover:border-neutral-900 hover:text-neutral-900' ?>"><?= h($f) ?></a>
      <?php endforeach; ?>
    </div>
  </div>
</section>
