...
.badge{display:inline-block;background:linear-gradient(135deg,#0d3b2e,#1a5c3a);color:#4ade80;padding:.2rem .6rem;font-size:.7rem;font-weight:900;margin-bottom:.5rem;border-radius:3px;letter-spacing:.02em}
...
<main style="max-width:1280px;margin:0 auto;padding:2.5rem 1.5rem 3rem">
  <div style="margin-bottom:1.75rem">
    <p style="font-size:.72rem;font-weight:800;color:#16a34a;letter-spacing:.08em;margin-bottom:.45rem">— ELECTRIC</p>
    <h1 style="font-size:2.2rem;font-weight:900;letter-spacing:-.02em">⚡ 전기차 <span style="color:#16a34a">렌트</span></h1>
    <p style="margin-top:.55rem;font-size:.95rem;color:#737373;font-weight:500">EV 보조금 적용 · 충전카드 포함 · 선납금 0원</p>
  </div>
...
<script>
const CARS = <?= json_js(get_ev_cars()) ?>;
  ...
  grid.innerHTML = list.map(c => `
    <div class="car-card">
      ...
      <div class="car-info">
        ...
        <button class="car-btn">전기차 견적 신청</button>
      </div>
    </div>`).join('');
}
renderCars(CARS);
