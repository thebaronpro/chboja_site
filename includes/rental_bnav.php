...
?>
<style>
/* === 모바일 하단 nav (둥근 floating 알약) === */
.mob-bottom-nav{
  position:fixed;bottom:.5rem;left:.75rem;right:.75rem;z-index:9999;
  background:#fff;border:1px solid #e5e5e5;border-radius:999px;
  box-shadow:0 8px 24px rgba(0,0,0,.12);
  padding-bottom:env(safe-area-inset-bottom);
  display:none;overflow:hidden;view-transition-name:mob-bnav;
}
@media(max-width:768px){.mob-bottom-nav{display:block}}
.mob-bottom-nav-inner{display:flex;align-items:stretch}
.mob-bottom-nav .bnav-item{position:relative;flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:.55rem .2rem .5rem;gap:.18rem;text-decoration:none;color:#525252;font-size:.65rem;font-weight:600;font-family:inherit;transition:color .15s}
.mob-bottom-nav .bnav-item.active{color:#2858E0}
.mob-bottom-nav .bnav-item.active::before{content:"";position:absolute;top:0;left:50%;transform:translateX(-50%);width:1.6rem;height:2.5px;background:#2858E0;border-radius:0 0 3px 3px}

/* 페이지 콘텐츠 하단 여유 — floating nav 가 가리지 않게 */
...
<nav class="mob-bottom-nav">
  <div class="mob-bottom-nav-inner">
    ...
    <a href="<?= h($site_root) ?>rental/limited.php" class="bnav-item<?= active_if($bnav_active === 'quick') ?>" data-tab="빠른출고">
